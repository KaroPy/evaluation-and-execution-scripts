"""
Fix incorrect Innkeepr - 90-180d Visitors - Exclusion configurations.

Reads the generated audit CSV, selects incorrect 90-180d visitor exclusions, and
updates:
...
from src.paths import (
    FIX_VISITORS_EXCLUSION_90_180D_DATA_DIR,
    FIX_VISITORS_EXCLUSION_90_180D_LOGS_DIR as LOGS_DIR,
    SCRIPT_DIR,
    SIGNAL_CONFIGURATION_DATA_DIR,
)

TARGET_EXCLUSION_NAME = "Innkeepr - 90-180d Visitors - Exclusion"
AUDIT_CSV_PATH = SIGNAL_CONFIGURATION_DATA_DIR / "signal_configuration_audit.csv"
PLAN_OUTPUT_PATH = (
    FIX_VISITORS_EXCLUSION_90_180D_DATA_DIR / "fix_90_180d_visitors_exclusion_plan.json"
)
...
def default_log_path() -> Path:
    ...
    return LOGS_DIR / f"fix_90_180d_visitors_exclusion_{stamp}.log"
...
def matches_90_180d_visitors_exclusion(audience_name: str) -> bool:
    return audience_name == TARGET_EXCLUSION_NAME or audience_name.startswith(
        f"{TARGET_EXCLUSION_NAME} -"
    )
...
def load_incorrect_90_180d_visitors(
    table: pd.DataFrame,
    workspaces: set[str] | None = None,
) -> pd.DataFrame:
    mask = table["audience.name"].apply(matches_90_180d_visitors_exclusion)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "comma-separated names, e.g. --workspace ESN --workspace Junglueck"
        ),
    )
    parser.add_argument(
        "--log-file",
        type=Path,
        default=None,
        help="Log file path (default: fix_90_180d_visitors_exclusion_<timestamp>.log)",
    )
...
def main() -> None:
    ...
    incorrect_rows = load_incorrect_90_180d_visitors(table, workspace_filter)
