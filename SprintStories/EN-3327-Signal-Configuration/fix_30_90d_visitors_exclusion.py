"""
Fix incorrect Innkeepr - 30-90d Visitors - Exclusion configurations.

Reads the generated audit CSV, selects incorrect 30-90d visitor exclusions, and
updates:
...
PLAN_OUTPUT_PATH = SCRIPT_DIR / "fix_30_90d_visitors_exclusion_plan.json"


def default_log_path() -> Path:
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    return SCRIPT_DIR / f"fix_30_90d_visitors_exclusion_{stamp}.log"
...
def matches_30_90d_visitors_exclusion(audience_name: str) -> bool:
...
def load_incorrect_30_90d_visitors(
    table: pd.DataFrame,
    workspaces: set[str] | None = None,
) -> pd.DataFrame:
    mask = table["audience.name"].apply(matches_30_90d_visitors_exclusion)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Log file path (default: fix_30_90d_visitors_exclusion_<timestamp>.log)",
    )
...
def main() -> None:
    ...
    incorrect_rows = load_incorrect_30_90d_visitors(table, workspace_filter)
